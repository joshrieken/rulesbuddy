defmodule RuleMavenWeb.AdminLive do
  ...
  alias RuleMaven.{Repo, Users, Games, Faq, InviteCodes}
  ...
  def handle_event("show_section", %{"section" => section}, socket) do
    ...
    invite_codes =
      if section == "invites", do: InviteCodes.list_codes(), else: socket.assigns.invite_codes
    ...
    <div style="margin:0;padding:0 0.2rem;width:100vw;position:relative;left:50%;right:50%;margin-left:-50vw;margin-right:-50vw">
      ...
      <div style="display:flex;gap:0.5rem;margin-bottom:0.5rem;align-items:center">
        ...
        <button
          type="button"
          phx-click="show_section"
          phx-value-section="invites"
          style={"background:none;border:1px solid var(--border);border-radius:0.3rem;padding:0.2rem 0.5rem;font-size:0.65rem;font-weight:600;cursor:pointer;#{if @section == "invites", do: "background:var(--accent);color:#fff;border-color:var(--accent)", else: "color:var(--text-muted)"}"}
        >Invite Codes</button>
        <button
          type="button"
          phx-click="show_section"
      ...
      <%!-- Invite codes --%>
      <%= if @section == "invites" do %>
        <div style="margin-top:1rem">
          <h2 style="font-size:1rem;font-weight:700;margin:0 0 0.5rem">
            Invite Codes ({length(@invite_codes)})
          </h2>
          <p style="font-size:0.75rem;color:var(--text-muted);margin:0 0 0.75rem">
            Generate invite codes for new users. Share a code — they'll use it to register.
          </p>
